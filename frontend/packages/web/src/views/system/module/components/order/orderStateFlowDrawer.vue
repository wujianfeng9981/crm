<template>
  <CrmDrawer v-model:show="show" :width="800" :footer="false" :title="t('module.order.stateFlowSet')">
    <div class="mb-[16px] text-[16px] font-semibold">{{ t('module.order.stateConfig') }}</div>
    ...
    <div class="mb-[16px] mt-[24px] text-[16px] font-semibold">
      {{ t('module.order.stateBackConfig') }}
    </div>
    <div class="flex items-center gap-[8px]">
...
<script setup lang="ts">
  ...
  const title = [t('module.order.state'), t('module.order.stateType')];
      ...
      return [
        {
          ...
          tooltipContent: element.stageHasData ? t('module.order.deleteUsedStatusTip') : '',
        },
      ];
    }
    return [
